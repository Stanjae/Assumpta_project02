...
const [registerLogin, setLoginStatus] = useState({message:'', status:false, color:'' })

const googleSignIn =async(data)=>{
  console.log(data, auth);
  try{
    await signInWithEmailAndPassword(auth, data?.email, data?.password);
    setLoginStatus({message:'Logged In successfully', status:true, color:'green'})
    setTimeout(()=> window.location.assign('/dashboard'), 3000)
  }catch(error){
    console.log(error)
    setLoginStatus({message:error?.code, status:true, color:'red'})
  }
}
  ...
  return (
    <AnimatedPage>
      <main className="w-full h-screen flex flex-col items-center justify-center px-4">
       {registerLogin.status && <Alerts alertInfo={registerLogin} setAlertInfo={setLoginStatus} />}
          <div className="max-w-sm w-full text-gray-600 space-y-5">
              ...
              <form
                  method='post' encType='multipart/form-data' onSubmit={handleSubmit( googleSignIn)}
                  className="space-y-5"
              >
                  <div>
                  ...
                  <button className="w-full text-white font-medium bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-600 flex items-center justify-center gap-x-3 py-2.5 border rounded-lg text-sm duration-150 ">

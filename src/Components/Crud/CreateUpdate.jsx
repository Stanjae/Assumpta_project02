...
const checkIFDataExists =(listofArr, searchString)=>{
    let i = false;
    listofArr.map((item)=>{
        if (item.fullname.toLowerCase().includes(searchString.fullname.toLowerCase() || item.email.toLowerCase() === searchString.email.toLowerCase())){
            i =  true; // If found, return true
        }
    })
    return i
}
...
const CreateUpdate = ({type, setAlertInfo}) => {
    ...
    const {addPriestToDb} = useFirebase();
    ...
    const onSubmit = async(data) => {
        ...
        console.log('dust: ', submittedData)
        if(checkIFDataExists(allPriests, submittedData) ){
            setAlertInfo({isAlert:true, message:'Sorry, Credentials already exists!!', color:'red'})
            setIsSubmitting(false)
            return;
        }
        try{
            await addPriestToDb(submittedData);
            await AddImages(imageData)
            setAlertInfo({isAlert:true, message:'Data submitted Sucessfully', color:'green'})
            setIsSubmitting(false)
        }catch(err){
            console.log(err)
  ...
  return (
    <div className=' bg-white/80 px-12 rounded-md py-10 '>
        <h2 className=' text-2xl py-3 font-semibold'> Priests Registration</h2>
        <form method='post' encType='multipart/form-data' onSubmit={handleSubmit(onSubmit)} className=' grid grid-cols-4 gap-5'>
            ...
            <div className=" relative md:col-span-2 col-span-4">
                ...
                <div className=' relative'>
                    ...
                    <input
                        {...register("details.email", { required: "This is required" })}
                        type="email"
                        placeholder="Enter your Email"
                        className="w-full pl-12 pr-3 py-2 text-gray-800 bg-transparent outline-none border focus:border-indigo-600 shadow-sm rounded-lg"
                    />
                </div>
                {errors.details?.email && <p className=' ms-4 text-sm text-red-500 font-medium'>{errors.details?.email?.message}</p>}
            </div>
            <div className=" relative md:col-span-2 col-span-4">
            ...
            <div className=" relative md:col-span-2 col-span-4">
                ...
                <div className=' relative'>
                    ...
                    <input
                        {...register("details.PlaceOfBirth", {required: "This is required"})}
                        type="text"
                        placeholder="Enter your Place of Birth"
                        className="w-full pl-12 pr-3 py-2 text-gray-800 bg-transparent outline-none border focus:border-indigo-600 shadow-sm rounded-lg"
                    />
                </div>
                {errors?.details?.PlaceOfBirth && <p className=' ms-4 text-sm text-red-500 font-medium'>{errors?.details?.PlaceOfBirth?.message}</p>}
            </div>
            <div className=" relative md:col-span-2 col-span-4">
                    ...
                    <div className=" relative md:col-span-2 col-span-4">
                        ...
                        <input className="block bg-opacity-10 top-0.5  absolute w-[98%] text-lg text-gray-900 rounded-lg cursor-pointer bg-gray-50 
                         focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                          id="large_size" {...register("details.academics.certiticate", {required:"This is required", validate:{
